  ...
  $.widget("dt.imageFeature", {
    ...
    _create: function () {
      ...
      if (opt.multiple == false) {
        ...
        this.input = $(
          '<input class="form-control form-control-sm" type="hidden" name="' +
            el_id +
            '" required="" value="' +
            opt.image +
            '"/>'
        ...
        this.image = $(
          '<img style="max-height:100px;max-width: 100%;cursor:pointer;"  src="' +
            opt.image +
            '" class="image_feature" />'
        ).appendTo(el);
      }
      ...
      this.modal = $(
        '<div aria-hidden="true" aria-labelledby="image-modalLabel" class="modal fade" id="image-modal-' +
          wigoID +
      ...
      this.template =
        ...
        '    <a href="#" class="rounded border d-inline-block image_tmp" data-id="{{id}}" data-src="' +
        '{{src}}">' +
    ...
    _bindEvents: function () {
      ...
      var opt = this.options;
      var event_remove;
      $(document).on("click", ".image_feature", function () {
        $(".elfinder", self.elfind)
          .elfinder({
